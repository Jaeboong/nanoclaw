// Renders markdown pipe tables as PNG buffers for Discord (additive module).
//
...
// PNG that Discord displays as an inline attachment.
//
...
// fall back to the original markdown table (no crash, no data loss).
//
// Ported faithfully from the v1 fork (src/channels/discord-table-render.ts);
// the only changes are the logger import (pino → v2 `log`) and font path
// resolution for the v2 tree. Lives under src/channels/discord-features/ as an
// additive module — see docs/UPSTREAM-MERGE.md.
...
function loadCanvas(): CanvasModule | null {
  ...
  try {
    ...
    if (!fontsDir) {
      ...
      log.warn('Discord table renderer: fonts directory not found');
      return null;
    }
    mod.GlobalFonts.registerFromPath(resolve(fontsDir, 'NotoSansMono-Regular.ttf'), FAM_MONO);
    mod.GlobalFonts.registerFromPath(resolve(fontsDir, 'NotoSansMono-Bold.ttf'), FAM_MONO_B);
    mod.GlobalFonts.registerFromPath(resolve(fontsDir, 'NotoSansKR-Regular.otf'), FAM_KR);
    mod.GlobalFonts.registerFromPath(resolve(fontsDir, 'NotoSansKR-Bold.otf'), FAM_KR_B);
    mod.GlobalFonts.registerFromPath(resolve(fontsDir, 'NotoEmoji-Regular.ttf'), FAM_EMOJI);
    canvasMod = mod;
    return mod;
  } catch (err) {
    loadError = err instanceof Error ? err.message : String(err);
    log.warn('Discord table renderer unavailable; falling back to markdown', { err: loadError });
    return null;
  }
}

function resolveFontsDir(): string | null {
  // Module lives at dist/channels/discord-features (compiled JS) or
  // src/channels/discord-features (tsx). Walk up to find assets/fonts at the
  // repo root.
...
export function parsePipeTable(lines: string[]): ParsedTable | null {
  const rows = lines.map((l) => splitPipeRow(l)).filter((r): r is string[] => r !== null);
  if (rows.length < 2) return null;

  // Drop alignment row if present (all cells match ^:?-+:?$).
  const alignmentIdx = rows.findIndex((row) => row.every((c) => /^:?-{2,}:?$/.test(c.trim())));
...
function drawLine(ctx: DrawingContext, line: WrappedLine, x: number, y: number): void {
  let cx = x;
  for (const r of line) {
    ctx.font = r.bold ? BOLD_FONT : REG_FONT;
...
function wrapCell(ctx: MeasuringContext, text: string, maxW: number, forceBold: boolean): WrappedLine[] {
  if (!text) return [[]];
...
export async function renderTableToPng(table: ParsedTable, opts: RenderOpts = {}): Promise<Buffer | null> {
  ...
  try {
    ...
    const wrap = (text: string, forceBold = false): WrappedLine[] => wrapCell(pctx, text, maxCol, forceBold);

    const headerLines = table.headers.map((h) => wrap(h, true));
    const rowLines = rows.map((r) => r.map((c) => wrap(c)));
    const overflowLines = overflowNote ? overflowNote.map((c) => wrap(c)) : null;
    ...
    const headerHeight = Math.max(...headerLines.map((l) => l.length)) * LINE_HEIGHT + CELL_PAD_Y * 2;
    const bodyHeights = rowLines.map(
      (row) => Math.max(...row.map((l) => l.length)) * LINE_HEIGHT + CELL_PAD_Y * 2,
    );
    const overflowHeight = overflowLines
      ? Math.max(...overflowLines.map((l) => l.length)) * LINE_HEIGHT + CELL_PAD_Y * 2
      : 0;

    const totalWidth = colWidths.reduce((a, b) => a + b, 0) + 1;
    const totalHeight = headerHeight + bodyHeights.reduce((a, b) => a + b, 0) + overflowHeight + 1;

    const canvas = mod.createCanvas(Math.ceil(totalWidth * scale), Math.ceil(totalHeight * scale));
    ...
    ctx.fillStyle = COLOR_HEADER_TEXT;
    {
      ...
      for (let c = 0; c < colCount; c++) {
        ...
        for (let li = 0; li < lines.length; li++) {
          drawLine(ctx, lines[li], x + CELL_PAD_X, CELL_PAD_Y + li * LINE_HEIGHT);
        }
        x += colWidths[c];
      }
    }
    ...
    for (let r = 0; r < rowLines.length; r++) {
      ...
      for (let c = 0; c < colCount; c++) {
        ...
        for (let li = 0; li < lines.length; li++) {
          drawLine(ctx, lines[li], x + CELL_PAD_X, y + CELL_PAD_Y + li * LINE_HEIGHT);
        }
        x += colWidths[c];
      }
      y += rowH;
    ...
    log.warn('Discord table render failed; falling back to markdown', {
